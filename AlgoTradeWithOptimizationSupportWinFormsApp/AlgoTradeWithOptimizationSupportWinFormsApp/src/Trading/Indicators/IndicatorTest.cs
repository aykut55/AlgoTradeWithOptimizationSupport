...
namespace AlgoTradeWithOptimizationSupportWinFormsApp.Indicators
{
    ...
    public class IndicatorTest
    {
        ...
        {
            _logManager.Log("========================================");
            _logManager.Log("INDICATOR TEST SUITE - STARTING");
            _logManager.Log("========================================");
            ...
            try
            {
                ...
                _logManager.Log("\n========================================");
                _logManager.Log("ALL TESTS COMPLETED SUCCESSFULLY!");
                _logManager.Log("========================================");
            }
            catch (Exception ex)
            {
                _logManager.LogError($"TEST FAILED: {ex.Message}", ex);
            }
        }
        ...
        private void Test_BasicSetup(List<StockData> data)
        {
            ...
            _logManager.Log($"✓ IndicatorManager created: {manager}");

            // Initialize with data
            manager.Initialize(data);
            _logManager.Log($"✓ Initialized with {manager.BarCount} bars");

            // Check initialization
            if (!manager.IsInitialized)
                throw new Exception("Manager not initialized!");
            _logManager.Log("✓ Manager is initialized");

            // Get cache stats
            var stats = manager.GetCacheStats();
            _logManager.Log($"✓ Cache stats: {stats["CacheSize"]}/{stats["MaxCacheSize"]}");
            ...
            _logManager.Log($"✓ Extracted prices: {closes.Length} close, {opens.Length} open, {highs.Length} high, {lows.Length} low, {volume.Length} volume");
        }
        ...
        private void Test_MovingAverages_Basic(List<StockData> data)
        {
            ...
            _logManager.Log($"✓ SMA(20): Last value = {sma20[^1]:F2}");

            var sma50 = manager.MA.SMA(closes, 50);
            _logManager.Log($"✓ SMA(50): Last value = {sma50[^1]:F2}");

            var sma200 = manager.MA.SMA(closes, 200);
            _logManager.Log($"✓ SMA(200): Last value = {sma200[^1]:F2}");

            // EMA
            var ema12 = manager.MA.EMA(closes, 12);
            _logManager.Log($"✓ EMA(12): Last value = {ema12[^1]:F2}");

            var ema26 = manager.MA.EMA(closes, 26);
            _logManager.Log($"✓ EMA(26): Last value = {ema26[^1]:F2}");

            // WMA
            var wma10 = manager.MA.WMA(closes, 10);
            _logManager.Log($"✓ WMA(10): Last value = {wma10[^1]:F2}");

            // Hull MA
            var hull20 = manager.MA.HullMA(closes, 20);
            _logManager.Log($"✓ Hull MA(20): Last value = {hull20[^1]:F2}");

            // DEMA
            var dema14 = manager.MA.DEMA(closes, 14);
            _logManager.Log($"✓ DEMA(14): Last value = {dema14[^1]:F2}");

            // TEMA
            var tema14 = manager.MA.TEMA(closes, 14);
            _logManager.Log($"✓ TEMA(14): Last value = {tema14[^1]:F2}");

            // VWMA (uses volume)
            var vwma20 = manager.MA.VWMA(closes, 20);
            _logManager.Log($"✓ VWMA(20): Last value = {vwma20[^1]:F2}");

            // LSMA (Linear Regression)
            var lsma20 = manager.MA.LSMA(closes, 20);
            _logManager.Log($"✓ LSMA(20): Last value = {lsma20[^1]:F2}");
        }
        ...
        private void Test_MovingAverages_Advanced(List<StockData> data)
        {
            ...
            _logManager.Log($"✓ KAMA(14,2,30): Last value = {kama14[^1]:F2}");

            // VIDYA (Variable Index Dynamic Average)
            var vidya14 = manager.MA.VIDYA(closes, 14);
            _logManager.Log($"✓ VIDYA(14): Last value = {vidya14[^1]:F2}");

            // ZLEMA (Zero-Lag EMA)
            var zlema20 = manager.MA.ZLEMA(closes, 20);
            _logManager.Log($"✓ ZLEMA(20): Last value = {zlema20[^1]:F2}");

            // T3 (Tillson T3)
            var t3_5 = manager.MA.T3(closes, 5, 0.7);
            _logManager.Log($"✓ T3(5, 0.7): Last value = {t3_5[^1]:F2}");

            // ALMA (Arnaud Legoux MA)
            var alma9 = manager.MA.ALMA(closes, 9, 6.0, 0.85);
            _logManager.Log($"✓ ALMA(9, 6.0, 0.85): Last value = {alma9[^1]:F2}");

            // JMA (Jurik MA)
            var jma7 = manager.MA.JMA(closes, 7, 0, 2);
            _logManager.Log($"✓ JMA(7, 0, 2): Last value = {jma7[^1]:F2}");
        }
        ...
        private void Test_MovingAverages_Bulk(List<StockData> data)
        {
            ...
            _logManager.Log($"✓ Generic MA - EMA(20): Last value = {ema20[^1]:F2}");

            var hull50 = manager.MA.Calculate(closes, MAMethod.HULL, 50);
            _logManager.Log($"✓ Generic MA - Hull(50): Last value = {hull50[^1]:F2}");
            ...
            _logManager.Log($"✓ Bulk SMA calculated for {periods.Length} periods:");
            for (int i = 0; i < periods.Length; i++)
            {
                _logManager.Log($"  SMA({periods[i]}): {smaResults[i][^1]:F2}");
            }
            ...
            _logManager.Log($"✓ Bulk MA(20) calculated for {methods.Length} methods:");
            for (int i = 0; i < methods.Length; i++)
            {
                _logManager.Log($"  {methods[i]}(20): {ma20Results[i][^1]:F2}");
            }
            ...
            _logManager.Log($"✓ Fibonacci EMAs calculated for {fibPeriods.Length} periods:");
            _logManager.Log($"  Periods: {string.Join(", ", fibPeriods)}");
            _logManager.Log($"  Last values: {string.Join(", ", fiboResults.Select(r => $"{r[^1]:F2}"))}");
        }
        ...
        private void Test_PriceUtils(List<StockData> data)
        {
            ...
            _logManager.Log($"✓ HHV(20): Last value = {hhv20[^1]:F2}");

            // LLV (Lowest Low Value)
            var llv20 = manager.Utils.LLV(lows, 20);
            _logManager.Log($"✓ LLV(20): Last value = {llv20[^1]:F2}");

            // Standard Deviation
            var stdDev20 = manager.Utils.StdDev(closes, 20);
            _logManager.Log($"✓ StdDev(20): Last value = {stdDev20[^1]:F4}");

            // Sum
            var sum20 = manager.Utils.Sum(closes, 20);
            _logManager.Log($"✓ Sum(20): Last value = {sum20[^1]:F2}");

            // Mean
            var mean20 = manager.Utils.Mean(closes, 20);
            _logManager.Log($"✓ Mean(20): Last value = {mean20[^1]:F2}");

            // Variance
            var variance20 = manager.Utils.Variance(closes, 20);
            _logManager.Log($"✓ Variance(20): Last value = {variance20[^1]:F4}");

            // True Range
            var tr = manager.Utils.TrueRange(highs, lows, closes);
            _logManager.Log($"✓ True Range: Last value = {tr[^1]:F2}");

            // Diff (price changes)
            var diff = manager.Utils.Diff(closes);
            _logManager.Log($"✓ Diff: Last value = {diff[^1]:F2}");

            // Percent Change
            var pctChange = manager.Utils.PercentChange(closes);
            _logManager.Log($"✓ Percent Change: Last value = {pctChange[^1]:F2}%");
        }
        ...
        private void Test_CachePerformance(List<StockData> data)
        {
            ...
            _logManager.Log($"Initial cache stats: {string.Join(", ", manager.GetCacheStats().Select(kv => $"{kv.Key}={kv.Value}"))}");
            ...
            _logManager.Log($"✓ After 16 calculations: Cache={stats1["CacheSize"]}/{stats1["MaxCacheSize"]}");

            // Re-calculate same indicator (should hit cache)
            var sma20_1 = manager.MA.SMA(closes, 20);
            var sma20_2 = manager.MA.SMA(closes, 20);
            _logManager.Log("✓ Re-calculated SMA(20) twice - should hit cache on 2nd call");

            // Clear cache
            manager.ClearCache();
            var stats2 = manager.GetCacheStats();
            _logManager.Log($"✓ After ClearCache: Cache={stats2["CacheSize"]}/{stats2["MaxCacheSize"]}");

            // Re-calculate after clear (should miss cache)
            var sma20_3 = manager.MA.SMA(closes, 20);
            _logManager.Log("✓ Calculated SMA(20) after clear - should miss cache");
        }
        ...
        private void Test_MomentumIndicators(List<StockData> data)
        {
            ...
            _logManager.Log("TODO: Implement RSI, MACD, Stochastic tests");
            ...
            _logManager.Log($"✓ RSI(14): Last value = {rsi14.Current:F2}, Overbought={rsi14.IsOverbought}, Oversold={rsi14.IsOversold}");

            // MACD
            var macd = manager.Momentum.MACD(closes, 12, 26, 9);
            _logManager.Log($"✓ MACD(12,26,9): MACD={macd.CurrentMACD:F2}, Signal={macd.CurrentSignal:F2}, Histogram={macd.CurrentHistogram:F2}");
            _logManager.Log($"  Trend: {(macd.IsBullish ? "Bullish" : "Bearish")}");

            // Stochastic
            var stoch = manager.Momentum.Stochastic(14, 3);
            _logManager.Log($"✓ Stochastic(14,3): K={stoch.k[^1]:F2}, D={stoch.d[^1]:F2}");
            */
        }
        ...
        private void Test_TrendIndicators(List<StockData> data)
        {
            ...
            _logManager.Log("TODO: Implement SuperTrend, MOST, ADX tests");
            ...
            _logManager.Log($"✓ SuperTrend(10,3.0): Value={supertrend.Current:F2}, Trend={(supertrend.IsBullish ? "Bullish" : "Bearish")}");

            // MOST
            var (most, exmov) = manager.Trend.MOST(21, 1.0);
            _logManager.Log($"✓ MOST(21,1.0): MOST={most[^1]:F2}, EXMOV={exmov[^1]:F2}");

            // ADX
            var adx = manager.Trend.ADX(14);
            _logManager.Log($"✓ ADX(14): Last value = {adx[^1]:F2}");
            */
        }
        ...
        {
            _logManager.Log("\n========================================");
            _logManager.Log("REAL-WORLD USAGE EXAMPLES");
            _logManager.Log("========================================");
            ...
            _logManager.Log($"SMA(50) = {lastSma50:F2}");
            _logManager.Log($"SMA(200) = {lastSma200:F2}");
            _logManager.Log($"Signal: {signal}");
            ...
            _logManager.Log($"EMA(20) = {ema20[^1]:F2}");
            _logManager.Log($"EMA(50) = {ema50[^1]:F2}");
            _logManager.Log($"EMA(100) = {ema100[^1]:F2}");
            _logManager.Log($"EMA(200) = {ema200[^1]:F2}");
            var trend = (ema20[^1] > ema50[^1] && ema50[^1] > ema100[^1] && ema100[^1] > ema200[^1])
                ? "STRONG UPTREND"
                : "NOT IN UPTREND";
            _logManager.Log($"Trend Analysis: {trend}");
            ...
            var price = closes[^1];
            _logManager.Log($"Price = {price:F2}");
            _logManager.Log($"BB Upper = {bbUpper:F2}");
            _logManager.Log($"BB Middle = {sma20[^1]:F2}");
            _logManager.Log($"BB Lower = {bbLower:F2}");
            _logManager.Log($"BB Width = {bbWidth:F2} (Volatility indicator)");

            var percentB = (price - bbLower) / (bbUpper - bbLower) * 100;
            _logManager.Log($"%B = {percentB:F2}% (0%=lower band, 100%=upper band)");
        }
